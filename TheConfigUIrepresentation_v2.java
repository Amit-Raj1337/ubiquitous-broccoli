...
public class TheConfigUIrepresentation_v1 {
    ...
    private static void visitChildNodesOfRoot(Document rootDocument) {
        ...
        for (int idx1 = 0; idx1 < nListTabs.getLength(); idx1++) {
            ...
            if ( attr!=null)  //making sure the attributes section isn't empty
            {
                Node p = attr.getNamedItem("name");  //node p now contains the value stored in name attribute

                d.addElement(generateTabButton(p,false));
            }

            d.addElement(visitChildUIElementsUnder_paramGroupTab(nListTabs, idx1));  //  SHOULD GO synonymous WITH "buildHTMLforTabPanels()"
     ...
     * @param idx1
     */
    private static div visitChildUIElementsUnder_paramGroupTab(NodeList nListTabs, int idx1) {
        div accordianDiv=new div();
        ...
        if (node.getNodeType() == Node.ELEMENT_NODE) {
           ...
           // if (size == 0)// return;
            ...
            ////  System.out.println("           " + nListAccordions.item(idx2).getAttributes().getNamedItem("name"));
                                   ...
                                   ////System.out.println(nListTabs.item(idx1).getAttributes().getNamedItem("name"));
                      Node n=nListAccordions.item(idx2);
                NamedNodeMap attr = n.getAttributes();
                ...
                if ( attr!=null)  //making sure the attributes section isn't empty
                {
                    ...
                    accordianDiv.addElement(generateTabButton(p,true));
                    //System.out.println(" --"+accordianDiv);
                }
    ...
    public static button generateTabButton(Node p, boolean checkIfAccordian)
    {
        if(checkIfAccordian) {
            button b = new button();
